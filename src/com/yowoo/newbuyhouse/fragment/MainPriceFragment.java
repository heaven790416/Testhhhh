...
public class MainPriceFragment extends MyFragment implements OnCameraChangeListener, 
OnMapReadyCallback{

	public static Fragment newInstance(Context context) {
		MainPriceFragment f = new MainPriceFragment();
	...
	MainPriceListFragment mainPriceListFragment;
	ViewPager pricePager, housePager;
	...
	//附屬的MarkerType: house
	private ArrayList<House> houseListForPager = new ArrayList<House>();
	private HashMap<Marker, HouseMarker> houseMarkerHashMap = new HashMap<Marker, HouseMarker>();
	...
	//for animation
	private Animation slideInFromBottomAnimation, priceSlideOutToBottomAnimation, houseSlideOutToBottomAnimation;
	
	//remark: outlier
	RelativeLayout remarkContainer;

	private final float MAX_ZOOM = 14.3f;
	private final float HOUSE_DISPLAY_ZOOM = 18.0f;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
		...
		try {
			root = inflater.inflate(R.layout.fragment_main_price, container, false);
		...
		remarkContainer = (RelativeLayout) root.findViewById(R.id.remarkContainer2);
		...
		housePager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				Log.e("test", "onPageSelected: "+position);
				if (position==houseListForPager.size()-1){
					fetchMorePriceByNOs(false);
				}
			}
		...
		//init views
		orderView.initItems(getResources().getStringArray(R.array.price_list_order_item));
		reloadBottomViews();
		...
		broadcastManager.registerReceiver(refreshPriceBroadcastReceiver, new IntentFilter(BHConstants.BROADCAST_PRICE_SEARCH_REFRESH_MAP));
		broadcastManager.registerReceiver(refreshPriceBroadcastReceiver, new IntentFilter(BHConstants.BROADCAST_PRICE_SEARCH_REFRESH_LIST));
	...
	public void onResume() {
		...
		reloadBottomViews();
		NewBuyHouseApplication.initTracker(getActivity(),"MainPriceFragment");
	}

	@Override
	public void onDestroy() {
		super.onDestroy();
		Log.e("test", "MainPriceFragment: onDestroy!");

		//((MainActivity) activity).onSectionAttached(getArguments().getInt(Constants.FRAGMENT_ARG_POSITION));

		LocalBroadcastManager broadcastManager = LocalBroadcastManager.getInstance(getActivity());
		broadcastManager.unregisterReceiver(refreshPriceBroadcastReceiver);
	}
	...
	private void setZoomAndStartMoveCamera(){
		...
		if (PriceSearchInfo.getInstance().currentSearchMode==BHConstants.SEARCH_MODE_LATLNG){
			zoom = MAX_ZOOM;
			Log.e("test", "startFetchBySearchMode: zoom1: "+zoom);
		}else if (PriceSearchInfo.getInstance().currentSearchMode==BHConstants.SEARCH_MODE_LOCATION){
			if (PriceSearchInfo.getInstance().selectedRoad>0){
				zoom = 16.5f;
				Log.e("test", "startFetchBySearchMode: zoom2: "+zoom);
			}else if (PriceSearchInfo.getInstance().selectedArea>0){
				//行政區有指定
				zoom = 15.0f;
				Log.e("test", "startFetchBySearchMode: zoom2: "+zoom);
			}else if (PriceSearchInfo.getInstance().selectedCity>0){
				//城市有指定
				zoom = MAX_ZOOM;
				Log.e("test", "startFetchBySearchMode: zoom3: "+zoom);
			}else{
				//未指定區域
				zoom = MAX_ZOOM;
				Log.e("test", "startFetchBySearchMode: zoom3: "+zoom);
		...
		moveCamera(PriceSearchInfo.getInstance().centerPos, zoom);
	...
	private void fetchPriceFromServer(){
		...
		String filterParams = PriceSearchInfo.getInstance().getFilterParams(false);
				...
				if (success){
					...
					if (getActivity()!=null){
						...
						if (priceMarkers.size()==0){
							showActivityToast(R.string.no_price_result_under_filter);
							return;
						}
	...
	private void setUpListIfNeeded() {
		if (mainPriceListFragment != null) {
			return;
		}
		mainPriceListFragment = ((MainPriceListFragment) getChildFragmentManager().findFragmentById(R.id.listFragment));
	}

	protected Fragment getListFragment(){
		setUpListIfNeeded();
		return mainPriceListFragment;
	}
	...
	private void startFetchListFromFilter(){
		...
		if (currentDisplayMode==BHConstants.HOUSE_MODE_MAP){
			...
			//切換至list
			mainPriceListFragment.reloadData();
			this.switchToList();
	...
	private void switchDisplayMode(){
		if (currentDisplayMode==BHConstants.HOUSE_MODE_MAP){
			...
			if (PriceSearchInfo.getInstance().currentFilterMode==BHConstants.FILTER_MODE_AREA){
				...
				PriceSearchInfo.getInstance().centerPos = mainPriceListFragment.getCurrentFirstHouseLatLng();
				setZoomAndStartMoveCamera();
	...
	private void setListener(){
			...
			public void onClick(View v) {
				if (currentDisplayMode==BHConstants.DISPLAY_MODE_MAP){
					...
					if (orderView.getVisibility()==View.VISIBLE){
						...
						orderView.reloadView(PriceSearchInfo.getInstance().selectedOrder);
		...
		getMap().setOnMarkerClickListener(new OnMarkerClickListener(){
			...
			public boolean onMarkerClick(Marker marker) {
				//更新目前marker顏色
				if (priceMarkerHashMap.containsKey(currentMapMarker)){
				...
				//更新新的marker顏色
				if (priceMarkerHashMap.containsKey(marker)){
					...
					HouseMarker houseMarker = houseMarkerHashMap.get(marker);
		...
		orderView.setOrderViewListener(new OrderViewListener(){
			...
			public void onClickItem(int index, String selectedText) {
				PriceSearchInfo.getInstance().selectedOrder = index;
	...
	public void onCameraChange(CameraPosition cameraPos) {
		...
		if ((cameraPos.zoom>HOUSE_DISPLAY_ZOOM) && Singleton.preferences.getBoolean(BHConstants.PREF_PRICE_SWITCH, false)){
			fetchHouseFromServer();
		}else{
			cleanHouseMarker();
		}
	}
	...
	/* Broadcast Receiver */
	private BroadcastReceiver refreshPriceBroadcastReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			Singleton.log("MainPriceFragment: get broadcast receiver!");

			try {
				String action = intent.getAction();
				//搜尋map
				if(BHConstants.BROADCAST_PRICE_SEARCH_REFRESH_MAP.equals(action)) {
					//refresh map, close list, get data
					startFetchMapFromFilter();
				}else if (BHConstants.BROADCAST_PRICE_SEARCH_REFRESH_LIST.equals(action)){
					//refresh list, get data
	...
	private void resetHouseMarkerIcon(Marker marker, Boolean isPressed){
		if (marker==null) return;
		if (houseMarkerHashMap.get(marker)==null) return;
	...
	/* House */
	private void cleanHouseMarker(){
		for(Entry<Marker, HouseMarker> entry : houseMarkerHashMap.entrySet()) {
			Marker key = entry.getKey();
			key.remove();
		}
		houseMarkerHashMap.clear();
	}
	
	private void fetchHouseFromServer(){
		VisibleRegion vr = mMap.getProjection().getVisibleRegion();
		float left = (float) vr.latLngBounds.southwest.longitude;
		float top = (float) vr.latLngBounds.northeast.latitude;
		float right = (float) vr.latLngBounds.northeast.longitude;
				...
				if (success){
					...
					if (getActivity()!=null){
						...
						for (int i=0; i<data.size(); i++){
							...
							houseMarkerHashMap.put(marker, data.get(i));
						}
					}
				}else{
					Log.e("test", "getHouseClusterMarkers fail!");
					showActivityToast(R.string.no_network_please_check);
				}
			}
		});
	...
	PagerAdapter housePagerAdapter = new PagerAdapter() {
		...
		public Object instantiateItem(View container, int position) {
			...
			final House house = houseListForPager.get(position);
			houseRow.reloadCell(house);
			houseRow.setHouseRowCallback(houseRowCallback);

			//set prev & next arrow
			houseRow.reloadArrow(position, houseListForPager.size());
	...
	private void fetchMoreHouseByNOs(final Boolean refresh){
		...
		//special issue: 
		final int tempHouseListSize = (refresh)? 0: houseListForPager.size();

		//get all houseNOs
		if (currentMapMarker==null) return;
		HouseMarker houseMarker = houseMarkerHashMap.get(currentMapMarker);
		...
		HouseService.getSimpleHouse(NOs, new SimpleHouseCallback(){
			...
			public void onResult(boolean success,ArrayList<House> houses) {
				if (success){
					...
					if (refresh) houseListForPager.clear();
					houseListForPager.addAll(houses);
